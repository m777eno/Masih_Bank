...
#Pupose of program: Simulated banking to teach about better banking habits.
...
def main_menu(account_num): #Displays main menu and Lets the user select options
    ...
    if select_main == "1":
        ...
        references() #References does not need user information to be passed into it.
    else:
        print(">>>Invalid Entry.<<<\nPlease Try again.")
...
def loan_calc(account_num):
    ...
    if monthly_pay > float(savings_val):
        print("As it turns out, with the current balance in your savings account you would not be able to afford a monthly payment.\nGo back to savings and deposit some funds, then try again.")
        loans(account_num)
        ...
        if afford < 12:
            ...
            print("Since you cannot pay for this for a year, make sure you have a steady income.")
